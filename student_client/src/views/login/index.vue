        ...
        if (valid) {
          ...
          if (that.ruleForm.type === 'admin' || that.ruleForm.type === 'teacher') {
            ...
            axios.post("http://localhost:10086/teacher/login", form).then(function (resp) {
              ...
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:10086/teacher/findById?tid=" + that.ruleForm.id).then(function (resp) {
            ...
            axios.post("http://localhost:10086/student/login", form).then(function (resp) {
              ...
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:10086/student/findById?sid=" + that.ruleForm.id).then(function (resp) {
                  console.log("登陆页正在获取用户信息" + resp.data)
